...
export default function Navbar() {
  ...
  // Farcaster mini app: compact static navbar
  if (isInMiniApp) {
    return (
      <header className="sticky top-0 z-50 bg-[#0A0B0D] border-b border-white/[0.06]">
        <div className="flex items-center justify-between px-3 h-11">
          <Link to="/" className="flex items-center">
            <img src="/assets/logo-white.svg" alt="BreakBase" className="h-[44px]" />
          </Link>

          <button
            className="p-1.5 rounded-full text-[#9CA3AF] hover:text-[#F9FAFB] hover:bg-white/[0.06] transition-colors"
                ...
                {[...NAV_LINKS, ...MORE_LINKS].map((link) => {
                  ...
                  return (
                    <Link
                      ...
                      className={cnm(
                        'px-3 py-2 rounded-lg text-[14px] font-medium transition-all duration-150',
                        isActive
                          ? 'text-[#0052FF] bg-[#0052FF]/[0.08]'
                          : 'text-[#9CA3AF] hover:text-[#F9FAFB] hover:bg-white/[0.06]',
                      )}
                    >
                      {link.label}
                    </Link>
